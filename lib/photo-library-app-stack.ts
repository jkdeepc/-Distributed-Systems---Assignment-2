...
export class EDAAppStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const DLQ = new sqs.Queue(this, "record-del-q", {
      retentionPeriod: Duration.days(1),
    });

    const imageQueue = new sqs.Queue(this, "record-q", {
    ...
    const deleteInvalidFn = new lambdanode.NodejsFunction(this, "deleteInvalidFn", {
      runtime: lambda.Runtime.NODEJS_22_X,
      entry: `${__dirname}/../lambdas/delate_img.ts`,
      timeout: Duration.seconds(10),
      memorySize: 128,
    });
    ...
    const dataTopic = new sns.Topic(this, "MetadataTopic", {
      ...
      const adddataFn = new lambdanode.NodejsFunction(this, "add_dataFn", {
        ...
        entry: `${__dirname}/../lambdas/add_data.ts`,
        environment: {
          TABLE_NAME: imagesTable.tableName,
        },
      });
      ...
      const statusTopic = new sns.Topic(this, "StatusTopic", {
        displayName: "Review Topic Status ",
      });
  
      const notifyTopic = new sns.Topic(this, "NotifyTopic", {
        displayName: "Notice Photographer Topic",
      });
      ...
      const updateStatusFn = new lambdanode.NodejsFunction(this, "updateStatusFn", {
        ...
        memorySize: 128,
        timeout: Duration.seconds(5),
        entry: `${__dirname}/../lambdas/update_data.ts`,
        environment: {
          TABLE_NAME: imagesTable.tableName,
      ...
      const notifyPhotographerFn = new lambdanode.NodejsFunction(this, "notifyFn", {
        ...
        memorySize: 128,
        timeout: Duration.seconds(5),
        entry: `${__dirname}/../lambdas/notify_info.ts`,
        environment: {
